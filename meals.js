document.addEventListener('load', ()=>{ //whenever the page loads
    update();
})
...
//local storage items for favorite list and last input
...
//Updating counter with the current number of favorite items.
...
//Check if an ID is in a list of favorites
 ...
 function isFav(list, id) {//list- list of favorites
    let res = false;
...
//Generates a random string starting the generated string
...
toggleButton.addEventListener("click", function () {//to toggle the sidebar and display the list of favorite meals
    showFavMealList();
    sidebar.classList.toggle("show");//to show side bar
    flexBox.classList.toggle('shrink');//to adjust the flexbox
});



flexBox.onscroll = function () {//making changes while scrolling content while searching
...
async function mealList() {//to show meal list based on search input
    ...
    if (mealsData.meals) {
        ...
        document.getElementById('cardsHolder').innerHTML = html;
    }
}
...
function addRemoveToFavList(id) {//to add or remove a meal from the favorite list
...
async function showMealDetails(itemId, searchInput) {// to show details for a specific meal
            ...
            <div class="fixed" id="searchBar">
                ...
                <div class="newSearchInput">
                    <form onkeyup="mealList()">
                        <input id="searchInput" type="text" placeholder="Search food, receipe" />
                    </form>
                </div>
            </div>
        </div>
    ...
    <div id="cardsHolder" class=" remove-top-margin ">`
    }
...
async function showFavMealList() {//to show all the meals which are added to the favourite list
